...
team_names = ('Astros', 'Yankees', 'Red Sox', 'Mariners', 'Indians', 'Cubs', 'Dodgers', 'Nationals', 'Brewers', 'Diamondbacks', 'Braves', 'Athletics', 'Cardinals',
...
class PitcherLeaderboard:
    ...
    def __init__(self, year):
        ...
        table = leaders_soup.find(lambda tag: tag.name == 'table' and tag.has_attr(
        ...
        if len(df) > 2:  # checks if any pitchers qualify
            ...
            df['SwStr%'] = (df['SwStr%'].str.replace('%', '')).astype(
                'double')  # removes % symbol from DF
            ...
            # fangraphs % stuff formats weird so I have to do this
            df['KN%'] = ((df['KN%'].str.replace('%', '')).astype('double'))/100
            df['KN%'] = pd.to_numeric(df['KN%'], downcast="float")

            # Finds Z-score values for calculation
            df['zxFIP-'] = (df['xFIP-'] - df['xFIP-'].mean()) / \
                df['xFIP-'].std(ddof=0)
            df['zSwStr%'] = (df['SwStr%'] - df['SwStr%'].mean()
                             )/df['SwStr%'].std(ddof=0)
            df['Strk'] = df['Strikes']/df['Pitches']  # calculates strike rate
            df['zStrk'] = (df['Strk'] - df['Strk'].mean()) / \
                df['Strk'].std(ddof=0)
            df['zFBv'] = (df['FBv'] - df['FBv'].mean())/df['FBv'].std(ddof=0)
            df['zAge'] = (df['Age'] - df['Age'].mean())/df['Age'].std(ddof=0)
            df['zPace'] = (df['Pace'] - df['Pace'].mean()) / \
                df['Pace'].std(ddof=0)
            # I do the calculation here becuase it helps when checking if the value meets the threshold or not
            df['Luck'] = (df['ERA-'] - df['xFIP-'])/20
            ...
            df['zAge'][df['zAge'] > 2] = 2
            df['Luck'][df['Luck'] > 1] = 1
            df['NERD'] = (-df['zxFIP-'] * 2) + -(df['zSwStr%'] / 2) + (df['zStrk'] / 2) + df['zFBv'] + \
                df['zAge'] + (-df['zPace']/2) + df['Luck'] + \
                (df['KN%'] * 5)  # base parts of NERD
            # I use feature scaling instead of adding a constant in order to get a more consistent scale of values
            df['NERD'] = ((df['NERD'] - min(list(df['NERD']))) /
                          (max(list(df['NERD'])) - min(list(df['NERD'])))) * 10
            self.df = df
            self.year = year
        else:
            raise ValueError(
                'No pitchers qualified for that year\'s leaderboard! Please try again.')

    def pitcher_nerd_score(self, player_name):
        pitcher_df = self.df
        # checks if there are more than 25 pitchers on the list *and* player is on the list.
        if (len(pitcher_df) >= 26) & (player_name in list(pitcher_df['Name'])):
            ...
            # initializes new Leaderboard object from previous year
            pitcher_lb = Pitcher_Leaderboard(str(self.year - 1))
