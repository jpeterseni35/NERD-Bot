...
while True:
    ...
    try:
        p_leaderboard = PitcherLeaderboard(
            now.year)  # creates pitcher leaderboard
        t_leaderboard = TeamLeaderboard(now.year)  # creates team leaderboard
    except:
        try:
            p_leaderboard = PitcherLeaderboard(
                int(now.year) - 1)  # creates pitcher leaderboard
            t_leaderboard = TeamLeaderboard(
                int(now.year) - 1)  # creates team leaderboard
        except:
    ...
    try:
        games = []
        # iterates through the games in nerd_bot
        for game in mlbgame.day(now.year, now.month, now.day):
            if game.home_team == 'D-backs':  # MLB gameday formatting is pretty stupid, idk why they have "D-Backs" here
                # like seriously, no other site abbreviates just "d-backs"
                game.home_team = 'Diamondbacks'
            ...
            game.home_team_NERD = int(round(t_leaderboard.team_nerd_score(
                game.home_team), 0))  # NERD score calculations
            try:
                game.home_pitcher_NERD = int(
                    round(p_leaderboard.pitcher_nerd_score(game.p_pitcher_home), 0))
            ...
            game.away_team_NERD = int(
                round(t_leaderboard.team_nerd_score(game.away_team), 0))
            try:
                game.away_pitcher_NERD = int(
                    round(p_leaderboard.pitcher_nerd_score(game.p_pitcher_away), 0))
            except:
                game.away_pitcher_NERD = int(5)
            game.total_NERD = int(round(
                ((game.home_team_NERD + game.home_pitcher_NERD + game.away_pitcher_NERD + game.away_team_NERD)/4), 0))
            games = games + [game]

        games = sorted(games, key=lambda x: x.total_NERD,
                       reverse=True)  # sorts by NERD for games
    except:
    ...
    try:
        ...
        # writes to an HTML table for conversion to image
        days_games.to_html("todays_table.html", index=False)
        ...
        f.write(new_html)
        f.close()
        imgkit.from_url('todays_table.html', 'todays_table.png',
                        config=config, options=options)
        body = 'Top games for today by NERD\n'
        body += '%s (%s) vs. %s (%s), %s ET (NERD: %d)\n' % (str(days_games['Home Team'].iloc[0]), str(days_games['Home P'].iloc[0]), str(
            days_games['Away Team'].iloc[0]), str(days_games['Away P'].iloc[0]), str(days_games['Game Time'].iloc[0]), int(days_games['GM'].iloc[0]))
        body += '%s (%s) vs. %s (%s), %s ET (NERD: %d)\n' % (str(days_games['Home Team'].iloc[1]), str(days_games['Home P'].iloc[1]), str(
            days_games['Away Team'].iloc[1]), str(days_games['Away P'].iloc[1]), str(days_games['Game Time'].iloc[1]), int(days_games['GM'].iloc[1]))
        body += '%s (%s) vs. %s (%s), %s ET (NERD: %d)\n' % (str(days_games['Home Team'].iloc[2]), str(days_games['Home P'].iloc[2]), str(
            days_games['Away Team'].iloc[2]), str(days_games['Away P'].iloc[2]), str(days_games['Game Time'].iloc[2]), int(days_games['GM'].iloc[2]))
    except:
    ...
    # Puts pitchers into leaderboard, NERD leaders
    pitcher_df = p_leaderboard.df.sort_values(by=['NERD'], ascending=False)
    ...
    # this chunk also writes the table into an IMAGE
    pitcher_df.to_html("pitcher_leaders.html", index=False)
    ...
    f.write(new_html)
    f.close()
    imgkit.from_url('pitcher_leaders.html',
                    'pitchers_table.png', config=config)
    body = 'Top pitchers by NERD - %d/%d/%d \n' % (
        now.month, now.day, now.year)
    body += '%s - %d\n' % (pitcher_df.iloc[0]
                           ['Name'], pitcher_df.iloc[0]['NERD'])
    body += '%s - %d\n' % (pitcher_df.iloc[1]
                           ['Name'], pitcher_df.iloc[1]['NERD'])
    body += '%s - %d\n' % (pitcher_df.iloc[2]
                           ['Name'], pitcher_df.iloc[2]['NERD'])
    body += '%s - %d\n' % (pitcher_df.iloc[3]
                           ['Name'], pitcher_df.iloc[3]['NERD'])
    body += '%s - %d\n' % (pitcher_df.iloc[4]
                           ['Name'], pitcher_df.iloc[4]['NERD'])
    ...
    game_times = [datetime.datetime.strptime(
        time, '%I:%M %p') - datetime.timedelta(minutes=10) for time in list(days_games['Game Time'])]
    game_times = [game_time.replace(year=now.year) for game_time in game_times]
    game_times = [game_time.replace(month=now.month)
    ...
    for index, row in days_games.iterrows():  # This posts NERD scores when games are about to begin
        print('Waiting for %s vs. %s to start' % (str(
            days_games['Home Team'].iloc[index]), str(days_games['Away Team'].iloc[index])))
        pause.until(game_times[index])
        body = '%s (%s) vs. %s (%s) about to start - NERD Game Score of %d' % (str(days_games['Home Team'].iloc[index]), str(
            days_games['Home P'].iloc[index]), str(days_games['Away Team'].iloc[index]), str(days_games['Away P'].iloc[index]), days_games['GM'].iloc[index])
